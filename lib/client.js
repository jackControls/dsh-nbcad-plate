...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      'status.probe.ready': 'print probe built',
      'status.probe.missing': 'print probe not built (overlay falls back to Python)',
      'status.env.ok': 'pdftoppm ok · {python}',
      'status.env.noPdftoppm': 'pdftoppm missing (install poppler)',
      ...
      'action.needEngine': 'the engine must be reachable first',
    ...
    const zh = {
      ...
      'status.probe.ready': '图纸探测器已构建',
      'status.probe.missing': '图纸探测器未构建（叠加图退回 Python 实现）',
      'status.env.ok': 'pdftoppm 正常 · {python}',
      'status.env.noPdftoppm': '缺少 pdftoppm（请安装 poppler）',
    ...
    function StatusStrip({ t }) {
      ...
      return h('div', { className: 'nbcad-status' },
        ...
        h('div', { className: 'nbcad-status-row' },
          ...
          h('span', null, tr(t, v.probe.ready ? 'status.probe.ready' : 'status.probe.missing')),
          h('span', { className: 'nbcad-muted' }, v.environment.pdftoppm.path ? tr(t, 'status.env.ok', { python: v.environment.python.version || 'python3' }) : tr(t, 'status.env.noPdftoppm'))),
    ...
    function JobCard({ ctx, t, job, onRemove }) {
      ...
      return h('div', { className: 'nbcad-card' },
        h('div', { className: 'nbcad-card-head' },
          ...
          h('button', { className: 'nbcad-btn', onClick: () => ctx.sessions.open(job.sessionId) }, tr(t, 'job.open')),
